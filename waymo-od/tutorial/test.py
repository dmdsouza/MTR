...
# LIDAR_DATA_FILE = f'/scratch1/dmdsouza/lidar/training/{womd_original_scenario.scenario_id}.tfrecord'
...
# # Extracting only features from the lidar point clouds
# frame_points_feature = {}  # map from frame indices to features
# frame_i = 0

# # Function to get laser calibration
...
# # Extracting only features from each LiDAR frame in the scenario proto
# for frame_lasers in scenario_augmented.compressed_frame_laser_data:
#     points_feature_list = []  # List to store features for each frame

#     # Retrieving pose for transformation
#     frame_pose = np.reshape(np.array(
#         scenario_augmented.compressed_frame_laser_data[frame_i].pose.transform),
#         (4, 4))

#     # Extracting features for each laser in the frame
#     for laser in frame_lasers.lasers:
#         if laser.name == dataset_pb2.LaserName.TOP:
#             c = _get_laser_calib(frame_lasers, laser.name)
#             (_, points_feature,
#              _, points_feature_return2) = womd_lidar_utils.extract_top_lidar_points(
#                  laser, frame_pose, c)
#         else:
#             c = _get_laser_calib(frame_lasers, laser.name)
#             (_, points_feature,
#              _, points_feature_return2) = womd_lidar_utils.extract_side_lidar_points(
#                  laser, c)
...
# # Printing shapes of extracted features
# for frame_idx, features in frame_points_feature.items():
#     print(f"Frame {frame_idx}: Shape of features extracted: {features.shape}")
